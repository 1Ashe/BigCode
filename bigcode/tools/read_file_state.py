...
class ReadFileState:
    ...
    def refresh_after_write(self, path: Path) -> FileSnapshot:
        """写入成功后重新生成快照，并标记来源为 write。"""
        snapshot = make_snapshot(path, content_digest=None, offset=None, limit=None, source="write", is_partial_view=True)
        self.record_read(path, snapshot)
        return snapshot
    ...
    def _matches_disk(self, snap: FileSnapshot) -> bool:
        ...
        if stat.st_mtime_ns != snap.mtime_ns:
            # mtime 变化不一定代表内容变化；再算一次 hash 可以避免无意义地拒绝编辑。
